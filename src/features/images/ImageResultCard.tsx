  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-wrap items-center justify-end gap-1">
        <Button type="button" variant="ghost" size="sm" onClick={() => onRegenerate(record)} title={t("imageStudio.regenerate")}>
          <RefreshCw className="h-4 w-4" />
        </Button>
        <Button type="button" variant="ghost" size="sm" onClick={() => onVariation(record, result)} title={t("imageStudio.variation")}>
          <Sparkles className="h-4 w-4" />
        </Button>
        <Button type="button" variant="ghost" size="sm" onClick={() => downloadImage(result, `image-${record.id}-${result.index}.png`)} title={t("imageStudio.download")}>
