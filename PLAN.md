...
绿色便携 · 单文件部署 · 跨平台 支持· 桌面+Web双管理 · 国际化支持

一个入口，告别零散笔记。 多家AI服务商的API Key、模型、用量，集中管理、统一调用。

多渠道路由 + 三级容错。 一个入口访问多家服务商，对不稳定模型自动冷却恢复、故障转移——分组精确匹配 → 模型模糊匹配 → AUTO 组 fallback，使用无缝衔接。

5套协议自由转换。 接入一次，同时适配 OpenAI-compatible / OpenAI-Responses / Claude / Gemini / Azure 五套协议，下游任意应用直接对接（支持 Claude、Codex 接入使用）。

分组即路由。 模型可按用途自由分组（默认 auto），分组名本身可当模型名使用；也可不分组，用模型部分名称（如 gpt-5）自动匹配所有 gpt-5.x 变体。支持渠道/模型测速，自动获取最新模型列表。

用量一目了然。 实时 Dashboard + 请求日志 + Token 消耗统计，日志保留完整分析数据，便于定位问题。
