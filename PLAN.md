...
> 决策建议：继续沿用当前 Web Admin adapter 架构，不做大迁移。当前方向（同一套 React 页面 + `tauriApiAdapter` / `webAdminApiAdapter` 双实现）是正确的；Tauri 官方成熟机制主要用于补强桌面壳，Web Admin 侧应通过契约对齐、错误统一、能力降级和部署约束来成熟化。

- [ ] **Web Admin endpoint / adapter 契约对照表**:
    - **现状**: `ApiAdapter` 已定义统一接口，`webAdminApiAdapter` 覆盖 channels / usage / pool / tokens / settings / proxy / testChat / version / translation.getLatest 等主能力。
    - **已发现问题**:
        1. `webAdminApiAdapter.pool.backfillCatalogMeta()` 发送 `{ items: [...] }`，但 Rust `pool_handlers::backfill_catalog_meta` 期望直接接收 `Vec<CatalogMetaUpdate>` 数组。
        2. 同一接口前端当前映射为 `catalog_provider / catalog_model_id` 语义，后端期望 `provider_logo / release_date / model_meta_zh / model_meta_en`，字段契约不一致。
        3. `translation.translateAndRelay()` Web 端直接抛错，属于 desktop-only / runtime-only 能力，需在 UI 层前置禁用而不是点击后报错。
    - **要求**:
        1. 以 `src/lib/apiAdapter.ts` 为基线，逐项对照 `tauriApiAdapter`、`webAdminApiAdapter`、`src-tauri/src/admin/router.rs` 的 HTTP endpoint。
        2. 形成表格：方法名 / Tauri 参数 / Web HTTP 路径 / Web 请求体 / Rust handler 入参 / 返回结构 / 是否一致。
        3. 对不支持 Web 的能力标记为 `desktop-only` 或 `runtime-only`，UI 读取 capability 后提前禁用。
...
- [ ] **Web Admin 操作反馈与错误语义统一**:
    - **现状**: `webAdminApiAdapter` 和 `webAuth` 各自有错误分类；`sonner` Toaster 已接入，但页面级操作反馈未确认全量覆盖。
...
- [ ] **一键测速连通性验证改进**:
    - **当前问题**: `test_entry_latency` 只发送 "hi" 并丢弃响应体，仅记录时延；未验证模型是否实际理解请求、返回了合规内容或连接异常。
    - **改进目标**:
        1. 发送特定指令要求模型回复 "OK"（或简单确认词），解析响应 JSON 中的 `choices[0].message.content` 是否匹配预期的确认词，以判断模型真实可达且运作正常。
        2. 检查 HTTP 响应状态码是否正常（2xx），异常时直接标记失败。
        3. 根据测速结果自动决策：
            - **连通且响应正确** → 清除该条目的冷却状态，若 `enabled` 被自动关闭则重新开启（考虑到 `enabled` 语义，可自动开启或提示用户）。
            - **失败（无响应/状态码异常/内容不符）** → 记录失败次数，达到阈值则自动关闭该条目（将 `enabled` 设为 false 或触发长冷却），避免持续故障影响用户体验。
    - **涉及文件**: `src-tauri/src/services/pool_service.rs` (test_entry_latency), `src-tauri/src/admin/pool_handlers.rs` (test_latency), 可能新增辅助解析函数。
    - **前端联动**: 测速完成后通过 Tauri 事件或命令结果通知前端更新 UI 状态（如开关状态），无需手动刷新。
...
- [x] **Web 管理端**: 已完成。同一套 `src/App.tsx` UI 通过 adapter 同时服务桌面（Tauri invoke）和浏览器（HTTP fetch）。浏览器登录后走 `webAdminApiAdapter`，桌面走 `tauriApiAdapter`。构建入口 `vite.config.web.ts`，产物 `dist-web-admin/`。登录/token 完整。架构详见下方 §15。
...
- 运行时选择：`isTauriRuntime() ? tauriApiAdapter : webAdminApiAdapter`
...
| `src/lib/useApiAdapter.ts` | `isTauriRuntime()` 检测 + adapter 选择 |
| `src/lib/webAdminApiAdapter.ts` | HTTP adapter，Bearer token 自动注入 |
| `src/lib/tauriApiAdapter.ts` | Tauri invoke adapter |
...
```text
Desktop: App.tsx → useApiAdapter() → tauriApiAdapter → invoke → Rust commands → Service → DB
Web:     App.tsx → LoginScreen → token → useApiAdapter() → webAdminApiAdapter → HTTP /admin/* → Rust handlers → Service → DB
```
...
- [x] 删除 `src/stubs/`、`src/index.html`、`vite.config.web.ts`（Web 构建临时文件）

### 15.6 后续待做

- [ ] `/admin/*` API prefix 是否迁移到根路径（当前保持 `/admin/*`）
- [ ] Standalone 模式验证（无 GUI 环境自动进入 Web Admin）
- [ ] 重建 Web 构建流程（vite.config.web.ts + stubs）
