...
> **目标**: 抽离独立渠道编辑器模块，统一渠道管理页、API 池/模型管理页点击渠道名称的编辑入口。

- [x] **独立模块抽离**:
    - **现状**: 已创建 `src/features/channels/editor/` 目录，包含独立编辑器组件。
    - **行为**:
        1. 渠道管理页新增/编辑渠道使用该模块。
        2. API 管理页点击渠道名称弹窗打开同一编辑器模块 (读取对应渠道数据)。
        3. 模型管理页点击渠道来源名称弹窗打开同一编辑器模块。
    - **当前范围**: 第一阶段保持前端弹窗打开逻辑和后端保存链路不变。
    - **未包含项** (后续阶段):
        - 后端组合保存 (渠道 + 模型关联的一站式保存)
        - 协议迁移 (编辑器协议类型选择逻辑优化)
        - 事实源治理 (渠道/模型数据一致性校验)
    - **需用户补充对齐的阻塞项** (2026-05-18):
        1. 协议事实值迁移：当前后端和数据库仍以 `custom` / `claude` 等旧值为事实源；如切换为文档中的 `openai` / `responses` / `anthropic` / `gemini` / `azure`，需要确认是否本阶段同步修改后端 adapter、数据库迁移、旧值 normalize、Web Admin HTTP 契约。
        2. 新增未落库模型测速：当前真实测速接口只支持已落库 `api_entries`；如要求临时创建模型保存前测速，需要新增以 `apiType + baseUrl + apiKey + modelName` 为输入的临时测速接口。
        3. 后端组合保存：第一阶段原定“不改后端协议”，但若要求保存过程完全事务化，需要确认是否提前实现 `save_channel_with_models`。
        4. 事实源治理：需要确认 `api_entries` 是否作为路由唯一事实源，以及 `channels.selected_models` 后续仅作 UI 快照还是继续参与业务判断。
