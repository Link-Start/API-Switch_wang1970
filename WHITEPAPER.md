...
API Switch 当前不是单一桌面程序，而是一套 core runtime 加多个壳层形态。后续开发必须先判断能力归属，再决定实现位置，禁止按报错点补丁式修复。

| 形态 | 壳层 | 必须启动 | 可选能力 | 禁止事项 |
|------|------|----------|----------|----------|
| Desktop | Tauri desktop shell | core、ServerApi、proxy、Admin/Web service、Desktop UI | tray、auto-launch、process/plugin、desktop connection apps | 将业务逻辑直接写入 shell command 并绕过 ServerApi |
| Headless / No-shell | 无壳 | core、ServerApi、proxy、Admin HTTP、embedded Web Admin | 无事件或 no-op event | 加载 Tauri/GTK/WebKit/AppHandle；用 fake shell handle 修业务问题 |
| Android Mobile | Tauri mobile shell | core、ServerApi、proxy、Android WebView UI | Android app data dir、Android 平台设置、后台提示、前台服务入口 | Android command 承载 channel/pool/token/config 等业务管理动作 |
| Web Admin | 浏览器 HTTP UI | Admin HTTP API、static Web Admin | connection apps 按 capability 暴露 | 直接依赖 Tauri event 或 Tauri-only API |

当前架构检查结论：

- `src-tauri/Cargo.toml` 已拆出 `core`、`gui`、`desktop`、`mobile`、`tray` feature，方向是编译期形态治理，不是运行时补丁。
- `src-tauri/src/lib.rs` 中 `run_gui()` 同时承担 Tauri builder、DB/AppState、Admin/Proxy 启动、托盘、插件、invoke handler 等职责，是后续补丁堆积风险点。
- `src-tauri/src/lib.rs` 中 `run_headless()` 已独立启动 core/proxy/Web Admin，但与 GUI 启动存在初始化重复。
- `src-tauri/src/runtime_plan.rs` 已定义 Desktop/Mobile/NoShell 三套 RuntimePlan，但当前主要是计划锚点，后续应升级为启动流程的单一计划来源。
- `src-tauri/src/server_api/mod.rs` 使用 `Option<AppEventHandle>` 表示 shell 事件能力可选，业务逻辑不得要求 shell handle 存在。
- `src/lib/unifiedApiAdapter.ts` 已统一前端调用入口，但每个业务方法仍手写 Tauri/Web 分支，新接口必须同时检查两侧映射，避免漂移。
...
| 能力类型 | 归属 | 暴露方式 | 备注 |
|----------|------|----------|------|
| 渠道 CRUD、模型选择、测速状态写入 | ServerApi | Admin HTTP + Tauri 兼容薄转发 | direct test/fetch 如不改状态，必须明确标注为独立直连探针 |
| 模型池 CRUD、排序、启停 | ServerApi | Admin HTTP + Tauri 兼容薄转发 | 不允许 HTTP 与 Tauri 分别实现业务流程 |
| Token 管理 | ServerApi | Admin HTTP + Tauri 兼容薄转发 | 状态变更统一 dirty/version |
| 设置读取、更新、patch | ServerApi | Admin HTTP + Tauri 兼容薄转发 | 设置变更涉及 proxy/admin 重启时也必须保持 Headless 无壳可用 |
| 导入/导出 | ServerApi | Admin HTTP + Tauri 兼容薄转发 | Headless 必须在 `app_handle = None` 时可用 |
| 代理启动、停止、状态 | ServerApi 或 core service facade | Admin HTTP + Tauri 兼容薄转发 | 需明确事件/dirty 归口 |
| 使用日志、统计读取 | service/read model | Admin HTTP + Tauri 兼容薄转发 | 读路径可不全部进入 ServerApi，但必须保持双端语义一致 |
| 托盘、窗口、auto-launch、process plugin | Desktop shell | Tauri command/plugin | Desktop-only |
| Android 设置、后台提示、前台服务 | Android shell | Tauri mobile command/plugin | Android-only，不读写业务状态 |
| Dirty/version 刷新 | core dirty state | Desktop `take_dirty`；Web `/state-version` | UI 统一 `useDirtyPolling` + React Query |

开发注意事项：

1. Headless 问题禁止通过加载 Tauri 壳、GTK/WebKit、fake AppHandle 解决；应修正依赖方向，让业务能力在无壳状态下成立。
2. Android 平台能力只处理系统设置、后台运行提示、前台服务等壳层动作，禁止承载业务管理流程。
3. Tauri command 不得成为新业务逻辑主入口；新增业务先进入 ServerApi，再由 Admin HTTP 与 Tauri command 做薄转发。
4. 前端不得绕过 `apiAdapter` 直接使用 Tauri-only 数据刷新或 HTTP-only 临时接口。
5. 跨形态问题修复前必须先回答：该能力属于 core、ServerApi、Admin HTTP、还是 shell；哪些形态应该拥有它；用哪组验证证明其他形态没有回归。
6. `RuntimePlan` 后续应成为运行入口的单一计划来源，避免 Desktop、Headless、Android 各自复制初始化流程。

最小验证矩阵：

| 变更范围 | 必须验证 |
|----------|----------|
| 前端或 UI 适配 | `pnpm typecheck`、`pnpm build:web-admin` |
| Rust 业务、ServerApi、Admin HTTP、commands、运行入口 | `cargo check --manifest-path src-tauri/Cargo.toml --features desktop`、`cargo check --manifest-path src-tauri/Cargo.toml --no-default-features` |
| mobile feature、数据目录、平台能力、Android 壳边界 | `cargo check --manifest-path src-tauri/Cargo.toml --features mobile` |
| 发布或 Android 工程 | `pnpm build:web-admin`、必要时 `pnpm android:build` |
...
**目标**：捕获既未命中禁用状态码、也未命中已知 `disable_keywords` 的失败原文，供人工分析后增补关键词库，形成"关键词越健壮 → 进入重试的失败越少"的正循环。

**触发条件**：转发失败 **且** 不满足以下任何一项时记录：
- 命中 `circuit_disable_codes`（401/403/410）
- 命中 `disable_keywords` 中的任意关键词

**记录文件**：`<data_dir>/disable-keyword-candidates.txt`（与 SQLite DB 同级目录）

**格式**：每行 `{status} {归一化原文}`，例如：
```
...
**去重**：按"状态码 + 归一化原文"（trim + 转小写 + 压缩连续空白）自动去重，重复行直接跳过，文件不膨胀。

**容错**：写入失败仅记录 `warn` 日志，绝不影响转发主流程。

**使用流程**：
1. 运行一段时间后查看 `disable-keyword-candidates.txt`
2. 挑出反复出现的固定错误句式（如 `"upstream rate limit exceeded"`）
3. 加入 `settings.disable_keywords`（设置页面 → 熔断机制 → 错误关键词匹配）
4. 下次同类失败被①直接拦截、不再记录，候选文件越来越短
5. 最终留下的都是真·网络抖动或偶发异常

**代码位置**：`src-tauri/src/proxy/keyword_log.rs` + `forwarder.rs` 失败分支 else 路径

---

#### 模型波动重试机制（v0.8.18+）

**目标**：在"连接优先（任何失败立刻换模型）"和"模型优先（网络抖动原地重试再换模型）"之间提供可配置的权衡。

**配置项**：`settings.model_retry_count`（模型波动重试次数），默认 `0`，范围 `0-5`
- **0**（默认）= 连接优先，任何失败立刻换模型（保持现有行为）
- **3** = 模型优先，网络抖动类失败原地重试 3 次，全失败才换模型

**重试逻辑**（双层循环，单一代码路径）：

```
外层 for entry in entries:
  内层 loop (最多 1 + retry_budget 次):
    forward_single() → 
      成功 → record_circuit_success, 返回结果
      失败 →
        ① 禁用类（401/403/410 或 disable_keywords）
           → 冷冻/禁用，break 换下一个 entry（不重试）
        ② 网络抖动类（status==0 或 5xx）且 retry_count < budget
           → continue 原地重试（不计数、不冷却、不触发 L1/L2/L3）
        ③ 不可重试 或 重试用完
           → 记 keyword + cool_down_entry（计数+1）+ break
```

**关键约束**：
1. **重试期间不惩罚**：retry 过程中不调 `cool_down_entry`，不碰 `failure_counts`、`cooldown_until`、L2 熔断器
2. **重试全失败才惩罚一次**：N 次重试耗尽后才调一次 `cool_down_entry`，保证"一次逻辑失败 = 一次计数"
3. **禁用类优先级最高**：401/关键词类失败不进重试，直接禁用/冷冻，符合"优先换模型"策略
4. **与 keyword 正循环兼容**：keyword 越健壮 → ① 拦得越多 → 进②重试的越少 → 重试只处理真波动

**可重试失败定义**（`is_retriable(status: u16) -> bool`）：
- `status == 0`（连接失败、超时、网络层错误）
- `status in 500..=599`（上游服务端瞬时不可用）

**非流式请求验证**：失败日志（`log_usage`）每次尝试都记录一次，但失败计数（`failure_counts`）只在退出内层 loop 时 +1。

**设置入口**：**Settings → Circuit Breaker → Model Fluctuation Retry Count**（中文："模型波动重试次数"）

**代码位置**：`src-tauri/src/proxy/forwarder.rs` `forward_with_retry()` + `is_retriable()`
