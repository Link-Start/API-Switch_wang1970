...
("web_admin_password", "admin"),
    ...
    // Migrate default disable_keywords to include new upstream structural-failure keywords.
    // Only rewrite rows still equal to the previous default; preserve any user customization.
    conn.execute(
        "UPDATE config SET value = ?1 WHERE key = 'disable_keywords' AND value = ?2",
        rusqlite::params![
